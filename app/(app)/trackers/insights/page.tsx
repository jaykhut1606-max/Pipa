...
export default function InsightsPage() {
  ...
  // pediatrician" CTA (?print=1). We wait for state=ready so charts and
  // copy are in the print preview.
  useEffect(() => {
  ...
  return (
    <main className="flex-1 flex flex-col bg-cream pb-32 print:pb-0">
      <NavBar showBack backHref="/trackers" title="Insights" />

      <div className="container-app pt-5 flex flex-col gap-5">
        <div className="flex items-center justify-between gap-3 print:hidden">
          <GranularityToggle value={granularity} setValue={setGranularity} />
          <button
            type="button"
            onClick={() => window.print()}
            className="shrink-0 inline-flex items-center gap-1.5 rounded-pill bg-plum text-cream h-10 px-4 text-small font-medium hover:bg-plum/90"
          >
        ...
        <header className="hidden print:flex print:flex-col print:gap-1 print:pb-4 print:border-b print:border-bone">
          <p className="font-display text-h2 text-ink">
            {babyName}&rsquo;s 7-day report
          </p>
          <p className="text-small text-stone">
            Generated by Pippa · {new Date().toLocaleDateString()}
          </p>
        </header>
